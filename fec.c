...
static u8_t gf_exp[510];  /* index->poly form conversion table    */
static int gf_log[256];   /* Poly->index form conversion table    */
static u8_t inverse[256]; /* inverse of field elem.               */
 ...
 * usage in _addmul1().
 */
static u8_t gf_mul_table[256][256];

#define gf_mul(x,y) gf_mul_table[x][y]

#define USE_GF_MULC u8_t * __gf_mulc_
...
static void _addmul1(u8_t* dst, const u8_t* src, u8_t c, size_t sz) {
  USE_GF_MULC;
  const u8_t* lim = &dst[sz - UNROLL + 1];
 ...
 */
static void _matmul(u8_t* a, u8_t* b, u8_t* c, u32_t n, u32_t k, u32_t m) {
  u32_t row, col, i;
 ...
 */
static void _invert_mat(u8_t* src, u32_t k) {
  u8_t c, *p;
  u32_t irow = 0;
  u32_t icol = 0;
  u32_t row, col, i, ix;

  u32_t* indxc = (u32_t*) malloc (k * sizeof(u32_t));
  u32_t* indxr = (u32_t*) malloc (k * sizeof(u32_t));
  u32_t* ipiv = (u32_t*) malloc (k * sizeof(u32_t));
  u8_t *id_row = NEW_GF_MATRIX (1, k);

  memset (id_row, '\0', k * sizeof (u8_t));
  /*
 ...
 found_piv:
    ...
    if (irow != icol)
      for (ix = 0; ix < k; ix++)
        SWAP (src[irow * k + ix], src[icol * k + ix], u8_t);
    indxr[col] = irow;
    indxc[col] = icol;
     ...
     */
    id_row[icol] = 1;
    if (memcmp (pivot_row, id_row, k * sizeof (u8_t)) != 0) {
  ...
  for (col = k; col > 0; col--)
    if (indxr[col-1] != indxc[col-1])
      for (row = 0; row < k; row++)
        SWAP (src[row * k + indxr[col-1]], src[row * k + indxc[col-1]], u8_t);
}
 ...
 */
void _invert_vdm (u8_t* src, u32_t k) {
  u32_t i, j, row, col;
  u8_t *b, *c, *p;
  u8_t t, xx;
  ...
  for (i = 1; i < k; i++) {
    u8_t p_i = p[i];            /* see above comment */
...
void fec_free (fec_t *p) {
    assert (p != NULL && p->magic == (((FEC_MAGIC ^ p->k) ^ p->n) ^ ((u64_t)p->enc_matrix)));
    free (p->enc_matrix);
    free (p);
}

fec_t * fec_new(u16_t k, u16_t n) {
  u32_t row, col;
  u8_t *p, *tmp_m;
  ...
  retval->magic = ((FEC_MAGIC ^ k) ^ n) ^ ((u64_t)retval->enc_matrix);
   ...
   */
  memset (retval->enc_matrix, '\0', k * k * sizeof (u8_t));
 ...
 */
void build_decode_matrix_into_space(
    const fec_t* code,
    const u32_t* index,
    const u32_t k,
    u8_t* matrix) {
...
void
fec_decode(const fec_t* code,
           const u8_t** inpkts,
           u8_t** outpkts,
           const u32_t* index,
           size_t sz) {
  u8_t* m_dec = (u8_t*)alloca(code->k * code->k);
  u8_t outix=0;
  u8_t row=0;
  u8_t col=0;
  ...
  for (row=0; row<code->k; row++) {
    /* If the block whose number is i is present, then it is required to be in the i'th element. */
